...
Passageiro* invocar_passageiro(Passageiro* inicio, char* nome) {
    ...
    // Verificando se a memória foi alocada corretamente (sempre bom em C!)
    ...
    if (inicio == NULL) {
        ...
        // A regra de ouro da lista circular: se só tem um, ele aponta para ele mesmo.
    ...
    atual->proximo = novo_espirito;       // O antigo último agora aponta para o novo passageiro
    novo_espirito->proximo = inicio;      // O novo passageiro aponta para o início (fechando o círculo!)
...
Passageiro* buscar_passageiro(Passageiro* inicio, char* nome) {
    ...
    // Usamos um loop do...while para garantir que vamos processar o primeiro elemento,
    // e paramos quando o "atual" der a volta e chegar no "inicio" novamente.
    do {
    ...
    do {
        if (strcmp(atual->nome, nome) == 0) {
            ...
            if (atual == inicio && atual->proximo == inicio) {
                ...
                printf("Banimento: A alma de %s foi banida. O carrossel agora esta vazio.\n", nome);
            ...
            if (atual == inicio) {
                ...
                ultimo->proximo = inicio; // O último agora aponta para o novo primeiro, fechando o círculo!
                free(atual);
                printf("Banimento: A primeira alma (%s) foi banida. A roda continua...\n", nome);
                return inicio;
            }
...
// Extra: Função para visualizar o carrossel (ajuda muito a testar se tudo está ligado certo)
void observar_carrossel(Passageiro* inicio) {
    if (inicio == NULL) {
